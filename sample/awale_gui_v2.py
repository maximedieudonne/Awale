...
class Game(ctk.CTk):
    ...
    def make_GUI(self):
        ...
        for index_cell in range(12):
            cell_frame = ctk.CTkButton(
                    ...
                    font = SEEDFONT,
                    command=lambda idx=index_cell: self.play(idx)
                )
            if index_cell < 6:
    ...
    def is_player_win(self):
        win = False
        if self.scoreN >= 24 :
            self.player_label.configure(text="Le Joueur N a gagné !")
            win = True
        if self.scoreS >= 24 : 
            self.player_label.configure(text="Le Joueur S a gagné !")
            win = True
        return win
    ...
    def update_GUI(self):
        for idx in range(12):
            self.cells[idx].configure(text=self.seeds[idx])
        
        self.scoreN_label.configure(text= self.scoreN)
        self.scoreS_label.configure(text= self.scoreS)
     

        if self.player == 'S' :
            self.player_label.configure(text = " C'est au tour de joueur Sud ")
        if self.player == 'N' :
            self.player_label.configure(text = " C'est au tour de joueur Nord ")

    def play(self,idx_cell):
